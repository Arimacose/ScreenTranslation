...
升级前已经保存的模型 ID 会作为当前选项保留，以兼容既有配置。
...
4. 真机填写 Base URL/API Key 后，`GET /models` 返回的含空格或连字符模型 ID
...
13. 首次网络发送的数据流确认与 `PRIVACY.md` 一致。

截至 2026-08-01，本地已通过门槛 1–3；协议/调度单元测试覆盖 HTTPS URL
拒绝规则、401/404/429/503 分类、一次重试上限、timeout/DNS 脱敏、畸形 JSON、
空模型列表、模型 ID 保真、空译文、请求取消和 latest-wins。门槛 4–13 中依赖 Android Keystore、真实网络、
