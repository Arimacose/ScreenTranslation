...
- 首页在模型、通知和悬浮窗权限就绪后只提示用户先切换到目标页面，再从常驻通知启动识屏；
  就绪态不再从 APP 内发起 MediaProjection，避免用户授权后仍停留在配置页。
- 首页操作区与翻译浮层职责分离：识屏运行时不再额外显示第二个“停止”，停止操作只保留在
  当前翻译浮层；空闲就绪态同样不显示无效的停止入口。
- 首页就绪引导从弱化的小号辅助文字提升为 18sp、主题主色的强调提示，使“先切换目标页面、
  再从常驻通知开始识屏”的下一步在无主按钮时仍清晰可见。
- 区域框选阶段改由全屏选择层消费系统返回和底层页面手势，并在右上角提供显式“退出识屏”；
  有效框选完成后退出按钮消失，保留既有浮层工具栏的完整操作。
- 悬浮窗授权入口优先进入系统“显示在其他应用的上层”专用列表，并携带 package 与 Settings
  聚焦参数；HyperOS 3 增加权限 ID 25 的悬浮窗应用列表后备入口。

### Fixed

- 移除模型准备、通知权限返回和悬浮窗权限返回后的隐式连续启动链，确保捕获只由目标页面上的
  常驻通知显式发起。
- 保留应用详情和厂商权限编辑器作为后备路由，修复 HyperOS 首选入口停留在普通应用信息页、
  需要继续经过“权限管理”和“其他权限”的冗长路径。
- 已授予的通知和悬浮窗权限按钮现在使用灰底灰字禁用样式，不再以蓝色主操作外观暗示可点击。
- 小于 32dp 的区域框选现在立即显示 Toast，不再只更新框选阶段不可见的状态文本。

### Verification

- 小米 15 Pro / Android 16 / HyperOS `OS3.0.304.0.WOBCNXM` 实机验证通知启动、系统投屏授权、
  区域框选、`WLAN` 到“无线局域网”的 Lite 本地翻译、完整“重选区域”UI，以及停止后的服务、
  MediaProjection、虚拟显示和悬浮窗收敛。
- Lite、Full、Online Debug 单元测试、Lite lint、Lite Debug 构建和 `git diff --check` 通过；
  最终公开资产继续由签名 acceptance workflow 从同一 source commit 构建、真机验收并原样提升。
...
- 区域结果面板的五个操作从无提示的横向滚动单行调整为 3 + 2 等宽网格，确保“重选区域”
  与“停止”在窄屏和大字体下仍完整可见，并保持至少 48 dp 的触控目标。
