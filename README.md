...
> v2.3.0 候选版提供三套可切换界面：Apple 风格默认主题、MIUIX，以及支持可关闭
...
> 项目状态：正在推进面向单一设备/ROM 基线的 `v2.3.0`。运行基线为 Android 16 / API 36，`minSdk` 与
...
- 可拖拽框选识别区域，并可调节采样间隔。
- 框选结果面板可独立拖动且不改变裁剪区域；可原子冻结/继续原文译文，并按横竖屏分别
  保存、重命名、应用和删除归一化区域预设。预设只保存名称、方向和坐标。
...
- 全屏控制条支持暂停/继续、显示/隐藏、阅读模式、字号、透明度和停止；阅读模式保留每个
  block 的完整原文/译文、复制入口和原文位置高亮，隐藏标签数量始终可见。
