...
ScreenTranslation 是一个由用户主动启动的屏幕翻译工具。本文件描述当前开源代码的
...
- 用户选择的捕获模式，以及区域模式下的框选坐标；
- OCR 识别出的文本；
- 翻译结果；
- 用户选择的源语言、目标语言和采样间隔；
- 模型准备任务的模型坐标、阶段、字节进度、速度、ETA 和网络约束；
- Lite 或 Full edition 所需的本地翻译模型；
- Online edition 中的用户 API 地址、模型、API Key 和对应的数据流确认状态。

## 屏幕内容

- 屏幕帧始终只在应用进程内用于当前任务。区域模式只把用户框选范围送入 OCR；
  Experimental 全屏增量模式在内存中为整帧生成低分辨率亮度签名，只把变化分块送入
  本机 OCR，并保留本次会话所需的文字框坐标。Lite/Full 的 OCR 原文和译文也只在
  设备端处理；Online 的稳定 OCR 文本按下述边界发送到用户配置的 API。
- 当前生产代码不把截图、原文或译文写入文件、数据库、历史记录或应用日志。
- Online 请求直接连接用户填写并确认的服务主机，不上传截图二进制。
...
应用保存源语言、目标语言、捕获模式、采样间隔、空闲快捷通知开关和模型 Wi-Fi-only
约束等设置。模型准备任务还保存稳定任务 ID、阶段、字节进度和最近一次已验证文件 identity，
不保存模型内容之外的屏幕数据。Online 还保存 provider、用户 Base URL、
...
`no_backup` 目录，卸载对应 edition 时随应用数据删除。模型管理页可删除已经下载或
部分下载的翻译权重；暂停、网络中断或进程结束时可保留 `.part` 文件以继续下载，用户取消
或在模型管理页删除时可清理这些文件。选择区域、文字框坐标、屏幕签名、截图和识别历史均不持久化。
...
v2.2.0 的 Lite、Full 与 Online APK 不使用 ML Kit Translate。仓库中的 `benchmark`
build type 仍保留 ML Kit Translate 作为可复现实验对照，但该对照不属于三份
GitHub Release APK。

## Online 翻译请求

Online edition 只提供用户自带密钥（BYOK）路径，只接受 HTTPS Base URL，拒绝 URL
中的账号密码、query 与 fragment，并关闭 HTTP/HTTPS 重定向。应用不内置公共 API
Key、项目托管模型或维护者网关。

应用使用 `Authorization: Bearer` 连接用户选择的 OpenAI-compatible 服务。
用户主动点击“获取可用模型”时，应用向自动补全的
`GET Base URL/models` 发送 API Key，并读取响应 `data[].id`；该请求不包含 OCR
文字或截图。选择模型后的翻译请求发送至自动补全的
`POST Base URL/chat/completions`。区域模式按稳定后的整段文字请求；Experimental
全屏增量模式按达到稳定条件的变化文字块逐个请求。一次请求包含：

- 固定的纯翻译 system message；
- 稳定后的区域 OCR 文本或单个变化文字块；
- 用户选择的源语言、目标语言与模型 ID。
...
重新确认该数据流。重选区域、内容隐藏或息屏会暂停处理；全屏模式同时清空 block
队列与标签。停止服务会取消活跃请求并清理协调状态。应用只在内存保存小型
翻译缓存，进程结束后消失。
...
| 权限/能力 | 用途 |
|---|---|
| `INTERNET` | Lite/Full 下载固定翻译模型；Online 连接用户配置的 API |
| `ACCESS_NETWORK_STATE` | 展示连接状态和模型准备反馈 |
| `SYSTEM_ALERT_WINDOW` | 框选区域并在其他应用上方显示结果 |
| `POST_NOTIFICATIONS` | 显示运行通知和停止后的快捷启动通知 |
| `FOREGROUND_SERVICE_MEDIA_PROJECTION` | 在用户授权后维持屏幕共享任务 |
| `FOREGROUND_SERVICE_DATA_SYNC` | 在前台通知可见时继续模型下载与校验 |
