...
应用保存源语言、目标语言、捕获模式和采样间隔等设置。Online 还保存 provider、用户 Base URL、
...
部分下载的翻译权重。选择区域、文字框坐标、屏幕签名、截图和识别历史均不持久化。
...
v2.0.0 的 Lite、Full 与 Online APK 不使用 ML Kit Translate。仓库中的 `benchmark`
